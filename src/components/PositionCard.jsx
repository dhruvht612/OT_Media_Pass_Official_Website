...
const PositionCard = ({ position }) => {
  ...
  return (
    <div className="group bg-gradient-to-br from-gray-950/80 via-gray-950/60 to-gray-950/80 border border-[#e8e]/30 rounded-2xl shadow-lg p-6 hover:shadow-[0_0_30px_rgba(238,136,238,0.4)] hover:border-[#e8e]/80 transition-all duration-500 text-center">
      <div className="mb-4">
        <div className="flex flex-col items-center gap-3 mb-2">
          <div className="flex items-center justify-center gap-3 flex-wrap">
            <h3 className="text-xl font-bold text-[#e8e] group-hover:text-lavender transition-colors text-center">
              {position.title}
            </h3>
            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
              ...
              {position.status}
              {position.positionsAvailable && position.positionsAvailable > 1 && (
                <span className="ml-1">({position.positionsAvailable})</span>
              )}
            </span>
          </div>
      ...
      <p className="text-gray-300 mb-4 leading-relaxed text-center">{position.description}</p>
      ...
      {position.status === 'Open' && (position.requirements || position.responsibilities) && (
        <>
          <button
            ...
            className="w-full flex items-center justify-center gap-2 text-lavender hover:text-lavender-light transition-colors py-2"
          >
          ...
          {isExpanded && (
            <div className="mt-4 pt-4 border-t border-[#e8e]/20 space-y-4 animate-fadeIn text-center">
              {position.requirements && (
                <div>
                  <h4 className="text-sm font-semibold text-[#e8e] mb-2 flex items-center justify-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 448 512" className="w-4 h-4">
